...
class Robot:
    ...
    # cls나 self를 사용하지 않는 그냥 함수도 사용 가능하다.
    # 이런 경우는 일반적이지 않으므로 @staticmethod 데코레이터를 사용한다.
    # @staticmethod 데코레이터를 사용하지 않으면 클래스에서만 해당 함수에 접근이 가능하고
    # @staticmethod 데코레이터를 사용하면 인스턴스에서도 해당 함수에 접근이 가능하다.
    @staticmethod
